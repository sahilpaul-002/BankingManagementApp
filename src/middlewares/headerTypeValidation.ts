    ...
    try {
        ...
        if (["POST", "PUT", "PATCH"].includes(req.method)) {
            ...
            if (!contentType || !contentType.includes("application/json")) {
                throw new AppErrorClass(406, "INVALID_HEADER", "'content-type' header must be application/json")
            }
        }
        ...
        if (!fromPortal) {
            throw new AppErrorClass(406, "INVALID_HEADER", "'from-portal' MISSING OR NOT STRING")
        }

        // Validate X-API-Key header
        const xApiKey: string | null = checkStringHeader(req.headers, "x-api-key");
        if (!xApiKey) {
            throw new AppErrorClass(406, "INVALID_HEADER", "'x-api-key' MISSING OR NOT STRING")
        }

        // Validate Authorization header
        const authorizationHeader: string | null = checkStringHeader(req.headers, "authorization");
        if (!authorizationHeader) {
            throw new AppErrorClass(406, "INVALID_HEADER", "'authorization' MISSING OR NOT STRING")
        }
