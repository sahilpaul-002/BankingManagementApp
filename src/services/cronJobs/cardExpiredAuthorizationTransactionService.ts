...
const processExpiredCardTransactions = async (): Promise<void> => {
    ...
    try {
        ...
        // ---------------------------------------------------------------------
        // Find expired PENDING card transaction authorizations
        //
        // Only transaction_id is required by the expiry transaction.
        // ---------------------------------------------------------------------
        ...
        //
        // We intentionally process each transaction independently.
        //
        // If one transaction fails, the remaining transactions should still
        // be processed.
        // ---------------------------------------------------------------------

        for (
            const transaction
            of expiredTransactions
        ) {
            ...
            try {
                ...
                logger.info(
                    `Card transaction ${transaction.transaction_id} authorization expiry processed successfully`,
                    {
                        serviceName:
                            "CardTransactionAuthorizationExpiryCronService",
                ...
                logger.error(
                    error,
                    {
                        ...
                        message:
                            `Failed to expire card transaction ${transaction.transaction_id}`,
                    }
                );
            }
        }
        ...
        logger.error(
            error,
            {
                ...
                message:
                    "Failed to process expired card transaction authorizations",
            }
        );
    }
};
                ...
                try {
                    ...
                    logger.error(
                        error,
                        {
                            ...
                            message:
                                "Unexpected error in card transaction authorization expiry cron job",
                        }
                    );
