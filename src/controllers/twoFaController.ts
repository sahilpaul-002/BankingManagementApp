...
export const sendVerifyEmailCode = async (req: Request, res: Response): Promise<Response<successResponseJson> | void> => {
    try {
        ...
        const verifyEmailServiceResponse = await sendVerificationEmailService(req, res, aesDecryptedBodyData?.email)
        if (verifyEmailServiceResponse?.status !== "SUCCESS") {
            return res.fail("SERVICE_ERROR", "Sending email verification code failed", 400);
        }
        return res.success("Email verification code sent successfully", {}, 200)
    }
    catch (err) {
