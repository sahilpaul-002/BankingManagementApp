...
const executeFiatPayoutTransaction = async (
    ...
    try {
        ...
        const { payoutQuoteId, userId } = transactionData;
        ...
        if (!sourceWallet) {
            throw new ServiceError(`Source wallet details not found for ${payoutQuote.source_currency}`);
        }

        // Read wallet balances
        const currentAccountBalance = new Decimal(sourceWallet.account_balance?.toString() ?? "0");
        const currentAvailableBalance = new Decimal(sourceWallet.available_balance?.toString() ?? "0");
        const currentHoldingAmount = new Decimal(sourceWallet.holding_amount?.toString() ?? "0");

        // Validate wallet accounting consistancy
        const calculatedAvailableBalance = currentAccountBalance.minus(currentHoldingAmount);
        if (!calculatedAvailableBalance.eq(currentAvailableBalance)) {
            throw new ServiceError("Wallet balance inconsistency detected");
        }
        ...
        const newAvailableBalance = currentAvailableBalance.minus(payoutSourceAmount);
        ...
        const [payoutTransaction] = await fiat_payout_transactions.create(
            [
                {
        ...
        if (!payoutTransaction) {
            throw new ServiceError("Failed to create payout transaction");
        }
        ...
        const walletTransaction = {
            ...
            amount: mongoose.Types.Decimal128.fromString(payoutSourceAmount.toFixed(4)),
            // Source wallet amount has not fee deduction
            fee: mongoose.Types.Decimal128.fromString("0"),
            balance_before: mongoose.Types.Decimal128.fromString(currentAvailableBalance.toFixed(4)),
            balance_after: mongoose.Types.Decimal128.fromString(newAvailableBalance.toFixed(4)),
        ...
        if (quoteUpdateResult.modifiedCount !== 1) {
            throw new ServiceError("Payout quote could not be marked as executed");
        }
        ...
        return {
            ...
            data: {
                ...
                wallet_transaction_id: walletTransactionId.toString(),
                wallet_id: payoutQuote.wallet_id.toString(),
                source_currency: payoutQuote.source_currency,
                source_amount: payoutQuote.source_amount,
                destination_currency: payoutQuote.destination_currency,
                destination_amount: payoutQuote.destination_amount,
