...
const bankPayoutProcessingTransaction = async (transactionData: bankPayoutProcessingTransactionData) => {
    ...
    try {
        ...
        const updateInc: Record<string, number> = {
            [`wallets_details.${sourceWalletIndex}.account_balance`]: -Number(payoutAmount.toFixed(4))
        };
        const updateSet: Record<string, any> = {
            [`wallets_details.${sourceWalletIndex}.holding_amount`]: newHoldingAmount.toFixed(4)
        };

        // Daily transaction
        const daily = sourceWallet.daily_transaction;
        if (!daily || daily.date.toDateString() !== now.toDateString()) {
            updateSet[`wallets_details.${sourceWalletIndex}.daily_transaction.debit`] = payoutAmount.toFixed(4);
            updateSet[`wallets_details.${sourceWalletIndex}.daily_transaction.date`] = now;
        } 
        else {
            updateInc[`wallets_details.${sourceWalletIndex}.daily_transaction.debit`] = Number(payoutAmount.toFixed(4));
        }
        ...
        if (isSameMonth) {
            updateInc[`wallets_details.${sourceWalletIndex}.monthly_transaction.debit`] = Number(payoutAmount.toFixed(4));
        }
         else {
            updateSet[`wallets_details.${sourceWalletIndex}.monthly_transaction.debit`] = payoutAmount.toFixed(4);
            updateSet[`wallets_details.${sourceWalletIndex}.monthly_transaction.month`] = now.getMonth() + 1;
            updateSet[`wallets_details.${sourceWalletIndex}.monthly_transaction.year`] = now.getFullYear();
        }

        // Yearly transaction
        const isSameYear = sourceWallet.yearly_transaction?.year === now.getFullYear();
        if (isSameYear) {
            updateInc[`wallets_details.${sourceWalletIndex}.yearly_transaction.debit`] = Number(payoutAmount.toFixed(4));
        }
        else {
            updateSet[`wallets_details.${sourceWalletIndex}.yearly_transaction.debit`] = payoutAmount.toFixed(4);
            updateSet[`wallets_details.${sourceWalletIndex}.yearly_transaction.year`] = now.getFullYear();
        }
        ...
        if (walletUpdateResult.modifiedCount !== 1) {
            throw new ServiceError("Failed to finalize payout wallet balance");
        }
        ...
        return {
            status: "SUCCESS",
            data: {
                payout_transaction_id: payoutTransaction._id.toString(),

                source_amount: payoutTransaction.source_amount,
