    ...
    try {
        ...
        if (!cardTransaction) {
            ...
            return {
                ...
                data: {
                    transaction_id: transactionId,
                    message: "Card transaction is no longer pending or has not expired",
                },
            };
        }
        ...
        if (!walletDetails) {
            throw new NotFoundError("Active user wallet not found");
        }
        ...
        if (walletIndex === -1) {
            throw new NotFoundError(`Active ${cardTransaction.currency} wallet not found`);
        }
        ...
        const currentAccountBalance = new Decimal(wallet.account_balance?.toString() ?? "0");
        const currentAvailableBalance = new Decimal(wallet.available_balance?.toString() ?? "0");
        const currentHoldingAmount = new Decimal(wallet.holding_amount?.toString() ?? "0");

        // Validate wallet balances
        if (!currentAccountBalance.isFinite() || !currentAvailableBalance.isFinite() || !currentHoldingAmount.isFinite()) {
            throw new ServiceError("Invalid wallet balance values");
        }
        if (currentAccountBalance.isNegative() || currentAvailableBalance.isNegative() || currentHoldingAmount.isNegative()) {
            throw new ServiceError("Wallet balance cannot be negative");
        }

        // Validate wallet balances accounting
        if (!currentAccountBalance.equals(currentAvailableBalance.plus(currentHoldingAmount))) {
            throw new ServiceError("Wallet balance inconsistency detected before releasing card transaction hold");
        }
        ...
        if (!newAccountBalance.equals(newAvailableBalance.plus(newHoldingAmount))) {
            throw new ServiceError("Wallet balance inconsistency detected after releasing card transaction hold");
        }
            ...
            try {
                ...
                logger.error(
                    err,
                    {
                        ...
                        message: "Failed to abort card authorization expiry transaction",
                    }
                );
            }
        }
